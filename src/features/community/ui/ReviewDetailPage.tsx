...
export function ReviewDetailPage({ id }: { id: string }) {
  ...
  return <section className="cm-workspace cm-single"><article className="cm-detail da-glass">
    ...
    <div className="cm-detail-heading"><ReviewCategoryBadges categories={review.categories}/>{!review.isSynthetic && <span className="cm-experience-label">실제 경험담</span>}{review.region && <span className="cm-region-badge">📍 {review.region}</span>}{review.isSynthetic && <span className="cm-synthetic">{review.authorName === '곁 운영팀' ? '운영팀 예시 글' : '해커톤 테스트용 가상 후기'}</span>}<h1>{review.title}</h1><div><span>{review.authorName}</span><time>{toKstDate(review.createdAt)}</time>{review.taskType && <span>{review.taskType}</span>}<span><Icon name="heart" size={14}/> 도움 {review.helpfulCount}</span></div></div>
    <ReviewArticleContent review={review}/>
    <HelpfulButton review={review} onUpdate={setReview}/>
    <ReviewComments postId={review.id} isLocalGathering={review.taskType.startsWith('지역모임')}/>
...
export function ReviewArticleContent({ review }: { review: CommunityReview }) {
  ...
  return <div className="cm-review-article">
    ...
    {difficulty && <p>
      처음에는 {difficulty} 무엇부터 확인해야 할지 정리하는 데에도 시간이 조금 필요했습니다.
    </p>}
    {actionTaken && <p>
    ...
    {caution && <p className="cm-review-caution">※ {caution}</p>}
  </div>
}
